...
## 0-A. 数量契約（改訂19 正本値 — revision lock manifest の counts と一致必須）

本節の表は**改訂19の数量契約の正本**であり、実装指示書 v2（plans/impl-instructions-charm-combo-display-v2.md）の同名テーブル・plans/charm-combo-revision-lock.json の `counts` と機械照合される（同期計画 §6-5 検証手順8〜9）。**planRevision 以外の数量は改訂14から不変**（§0-C-3・§0-D-6・§0-E-10・§0-F-3）。
...
**〔改訂19 失効 — 以下は改訂17〜18 当時の歴史記録・現行は §0-G-3〕** swan が threshold 10 なのは、銀の線画ロゴは閾値を上げると断片化するため（th14 701px/4成分・th18 565px/7成分・th22 443px/11成分）。keep 除外の効果は成分構造で確認済み（除外なし 1,221px/6成分 → 除外あり 898px/5成分〔検出全体 bbox (383, 903) 49×52〕。**除外なしの6成分 ＝ 除外ありの5成分〔画素単位で同一〕＋ ストーン由来成分 323px / 1成分〔bbox (431, 926) 15×39〕であり、898 + 323 = 1,221 が厳密に成立する**。除外なし側の検出全体 bbox はストーン成分を含むため (383, 903) 63×62 となる。数値はいずれも修正後 `logoRoi` での実装忠実ハーネス実測 2026-08-02。なお threshold 掃引〔th14 / th18 / th22〕は keep 除外ありの値であり、logoRoi 修正の前後で完全同一であることを実測確認済み）。**なお上表の「ロゴ成分の bbox」49×53（y 903–955）と `detectLogo` の検出 bbox 49×52 の1px 差は、`detectLogo` の閾値判定で最下行のアンチエイリアス1行が落ちることによる**（`logoRoi` は成分 bbox 49×53 に ±14px を取っており両者を包含する）。

**検査の追加**: **Phase 3 selftest へ「全8種の logoPxCount と成分数が `logoRange` 内であること」「`detectLogo` を用いる6種の採用 threshold が契約値と一致し、horseshoe / osanpo は `logoThreshold` が `null` であること」を追加**し（**改訂19: swan も null へ移行し「`detectLogo` を用いる5種の実数値＋horseshoe / osanpo / swan の null」で読み替える — §0-G-3**）、**Phase 4 は全8種の logoPxCount・成分数・`logoThreshold`（6種は実数値・2種は null**〔改訂19: 5種は実数値・horseshoe / osanpo / swan の3種は null — §0-G-3〕**）を prep.json の `inspections17.logoInspection`（§5-4）と prep-inspection-1-8.json の双方へ、`keepPxCountWork`・`keepPxCount` を派生定義値（§5-4）として記録**する。範囲外・非該当種への実数値混入はいずれも生成エラー（fail-fast）。
...
## 0-F. 改訂18の変更点一覧（2026-08-10）

**Phase 4 検査8 実測の還流による再改訂**。rev17 の Phase 4 実行（専用 worktree 上のブラウザ実行・2026-08-10 報告）で、全8種が検査1〜7・M・F を通過して書き出しに至り、コミット前検査8（三者照合・§5-4）の**①入力照合・②出力照合は全8種 PASS** した一方、**③再生成照合が全8種 FAIL** した（検査8 が通らないため assetCommit は未作成 — v2 §6「8種すべて合格するまで派生アセットをコミットしない」のとおり停止）。還流票はない（Codex は rev17 Phase 3 evidence まで正常完了しており、本件は検査8 実測の報告に基づく再改訂である）。

**実測された失敗**:

| 対象 | 失敗 | 経路 |
|---|---|---|
| horse / horseshoe / frenchie / dachshund / toypoodle / osanpo / cat | `inspections17 fail-fast: logoRange` | 再生成後の inspections17 照合 — 検証 spec に `logoRange` が無く rangeCheck が恒偽（§0-E-10 の logoInspection 照合式） |
| swan | `Cannot read properties of undefined (reading 'backgroundDistance')` | くちばし監査の再生成（§0-E-6 のレシピ再実行）が検証 spec の `manualRecipe`（undefined）の `backgroundDistance` を参照して即時 TypeError |

**原因**: 検査8 ③再生成照合が用いる検証 spec の合成関数（`verificationSpecFromPrep` 相当）が、**prep.json 記録値のみから spec を構成し、CHARM_SPECS のツール定数を合成していなかった**。§0-D〜§0-E で CHARM_SPECS へ新設したツール定数（`manualRecipe`・全8種 `logoRange`・cat／swan `logoRoi`・frenchie `cropSource`・swan `beakRemoval`／`stoneRoi`／`stoneLumMax` 等）は prep.json スキーマの外にある（§0-D-3・§0-E-1 の境界契約）ため、prep.json 由来だけの spec では生成経路と同じ生成補助・検査が再実行できない。検査8 の入力照合・アーカイブ側 inspections17 照合は CHARM_SPECS を直接参照するため PASS し、**再生成側だけが破れる非対称**が生じていた。

**是正方針**: prep.json へレシピ・レンジ定数を追加記録する回避は**境界契約違反であり採らない**（§0-D-3・§0-E-1 — prep.json スキーマと生成規則は不変）。是正は検証 spec の合成側のみで行う（§0-F-1）。

**再改訂分類（§12-2）**: 変更の実体は検査8 の検証実装と selftest のみ（「schema/tool・検査実装」= Phase 3 の単独分類）。canonicalSource・freeze JSON・sizeMm・grain 素材・生成規則（パイプライン①〜⑧'）はいずれも不変のため Phase 1／2 に該当せず、manual レイヤ・base/masks/prep の生成契約変更も無いため Phase 4 分類にも該当しない。本改訂に伴う §5-4・§9-0・v2 指示書の記述更新は、この Phase 3 分類の変更を文書へ反映する §12-1 手順1 の同一改訂であり、Phase 2 分類の「計画権限・数量・実装指示・受け入れ基準**だけ**の変更」（ツール実装に影響しない文書上の変更）には該当しない。→ **restartPhase = Phase 3**。

### 0-F-1. 検査8 再生成spec合成契約（新設）

検査8 ③再生成照合（§5-4）の独立再実行に用いる検証 spec は、次の**2層の合成**で構成する。

**(a) 実測入力層 — prep.json 記録値が正本**:

| フィールド | 出所 | 再生成での使用先 | 既存検証 |
|---|---|---|---|
| pipeline 値（crop・workSize・assetSize・scale・background・logoRoiWork・stitchContrast） | prep.pipeline | 再生成の工程パラメータとして**記録値を直接使用**（①〜⑧' へそのまま渡す — 検証 spec の合成定数で置換しない） | 変換契約の再計算一致（§5-4 検査8 の既存照合）＋新設 assert ③④ |
| calibration・stitchPathsWork | prep.inputs | 検証 spec 経由（派生値の再計算・検査3／検査5） | calibration／stitchPaths の record 検証（§5-2） |
| grain（material 別実測レコード — **生成時の単一既定値とは形状が異なる map**） | prep.inputs.grain | 再生成の grain 適用は **prep 記録レコードを explicit 設定として直接使用**し、検証 spec.grain はこの map を保持する | 検査F-2 が契約定数部分（grainRadiusPx・grainSourceInset・shadeMin・tileMode・grainScaleMm）を契約値照合 |
| stitchMode・threadColor（**swan のみ**） | prep.inputs | 検証 spec 経由（検査5 の分岐・CHARMS 転記照合） | 検査5 の完全列挙型ゲート（null を含む未知値は即不合格） |

**swan 行の一意化（§0-D-4・§0-E-11 との接続）**: swan の stitchMode・threadColor は Phase 4 の判定で確定した**実効値**の記録であり、§0-D-4／§0-E-11 の「Phase 4 で確定した `CHARM_SPECS[key].stitchMode`」が指す確定後の実効値と**同一の値**である。**実効値の記録正本は prep.inputs**（検査5 の完全列挙型ゲートを通過した値）であり、確定前の CHARM_SPECS リテラル初期値（null）はプレースホルダであって検証 spec へ合成してはならない。検査5・§0-E-11 Phase 6 完了条件・CHARMS 転記の分岐判定は、この同一の実効値に対する述語であり矛盾しない。

**(b) ツール定数層 — CHARM_SPECS[key] が正本（prep.json スキーマの外・§0-D-3／§0-E-1 の境界契約）**:

| フィールド | 対象 | 合成 |
|---|---|---|
| label・source・sourceMode・toneMode・sizeMm・sizeMmMeasured・attachmentMode | 全8種 | CHARM_SPECS の値そのもの |
| threadColor・stitchMode | **swan を除く7種**（swan は (a) — 実効値の記録正本が prep.inputs のため、確定前のリテラル初期値 null を合成対象にしない） | CHARM_SPECS の値そのもの |
| logoThreshold・logoRoi・logoRange | 全8種（horseshoe／osanpo の logoThreshold は null・§0-E-8。**改訂19: swan も null — §0-G-3**） | CHARM_SPECS の値そのもの |
| manualRecipe | 全8種（6種 = `COLORLESS_CLOSED_WHITE`〔§0-E-1〕・horseshoe〔§0-D-3・§0-E-5〕・osanpo v2〔§0-E-4〕） | CHARM_SPECS の値そのもの |
| cropSource | frenchie（§0-D-2） | CHARM_SPECS の値そのもの |
| beakRemoval・stoneRoi・stoneLumMax | swan（§0-E-6・§0-E-8） | CHARM_SPECS の値そのもの |
| **logoTransplant・oldLogoRemoval（改訂19 新設 — §0-G-1／§0-G-2）** | swan | CHARM_SPECS の値そのもの |

**重複フィールドの一致 assert**（両層に現れる値は一致を機械照合してから (b) の値を採用する）:

- 既存（改訂17まで）: charmKey／label・sourceMode（§4-1c マニフェスト込み）・toneMode 対応・logoThreshold（pipeline.logoThreshold = CHARM_SPECS.logoThreshold）・source file（§4-1c マニフェスト照合）
- **新設① sizeMm・sizeMmMeasured・attachmentMode**: prep.inputs の記録値が CHARM_SPECS と完全一致（jsonEqual）でなければ検査8 FAIL
- **新設② threadColor・stitchMode（swan を除く7種）**: prep.inputs の記録値が CHARM_SPECS と完全一致でなければ検査8 FAIL（swan は検査5 の完全列挙型ゲートが正 — 一致対象外）
- **新設③ crop の一意化**: `cropSource` を持つチャーム（現行 frenchie のみ）は pipeline.crop が cropSource と完全一致（§0-D-2 の cropSource 契約の再生成側ゲート — 従来は生成側にのみ存在した）、**`cropSource` を持たないチャーム（現行 7種）は pipeline.crop が全面クロップ `{x: 0, y: 0, w: canonicalSource 原寸幅, h: canonicalSource 原寸高}` と完全一致**（原寸は読み込んだ canonicalSource の実寸 — §0-E-12 の原寸列と一致する）でなければ、いずれも検査8 FAIL。**これにより全8種の crop は機械的に一意（frenchie = cropSource・他7種 = 全面）となり、実装者間で異なる crop が選ばれる余地はない**
- **新設④ logoRoi → pipeline.logoRoiWork**: `logoRoiSource` を持たないチャーム（**現行 全8種**）は pipeline.logoRoiWork が CHARM_SPECS.logoRoi（比率）と完全一致でなければ検査8 FAIL（生成時の logoRoiWork は logoRoiSource 不在時 logoRoi をそのまま採用する恒等対応のため、一致 assert は jsonEqual で書ける。将来 `logoRoiSource` を導入するチャームは source 矩形 → 作業座標 ROI 変換の再計算一致で照合する）

**全面クロップ規範の根拠（新設③後段）**: 直近の完全バッチ（rev16 Phase 4 assetCommit `e71a1d80`）の prep.json 実績は、cropSource を持たない7種すべてが全面クロップである（horse {x:0, y:0, w:2158, h:2233}・dachshund {x:0, y:0, w:2195, h:2195}・cat {x:0, y:0, w:2195, h:2195}・osanpo {x:0, y:0, w:2304, h:2304} など — §0-E-12 の原寸列と一致）。本 assert はこの既成運用の契約明文化であり、§0-E の実測由来定数（logoRange・stitchRange・外周取り残し率など）の測定前提とも一致する — **生成規則・出力画素は変えない**。prep.json と出力を一貫して偽装した crop は②③の内部整合だけでは検出できないため、この明示的な crop 規範 assert が再生成側の一意性を保証する。

**実効 spec の投影規則（層の優先関係の一意化）**: 検証 spec の各フィールドの実効値は、帰属表（§0-F-2）で**ツール定数層に分類されるフィールドは CHARM_SPECS[key] から**、**実測入力層に分類されるフィールド（calibration・stitchPathsWork・grain・swan の stitchMode／threadColor）は必ず prep.json 記録値から**取得する。CHARM_SPECS 側に存在する同名フィールドの静的値（osanpo の calibration 契約座標・stitchPathsWork の初期値 []・grain の生成時既定値）は**生成時の seed／default であり、検証 spec の実効値として使用してはならない**。grain の帰属は leaf 単位でも一意である — 実効値（grainBase・grainAmp・grainScaleMm・grainSourceInset・shadeMin・effectiveWidth／effectiveHeight／effectiveRoiSha256 等の全 leaf）は prep.inputs.grain の記録値が正本であり、契約定数 leaf（grainRadiusPx・grainSourceInset・shadeMin・tileMode・grainScaleMm）の契約適合は検査F-2 が **prep 記録値に対して**照合する（CHARM_SPECS の既定値を検証 spec へ持ち込まない）。

**合成の不変式（ツール定数層に限定）**: 生成経路（§5-2 パイプライン①〜⑧'・§5-4 検査1〜7・M・F・§0-E-10 の inspections17）が CHARM_SPECS[key] から読むフィールドのうち、**帰属表でツール定数層に分類される全フィールド**は、検証 spec 上でも同一値で参照可能でなければならない（実測入力層フィールドは上記投影規則のとおり prep.json 実効値が正であり本不変式の対象外 — CHARM_SPECS の seed／default を合成する実装は selftest ⑬ (iii) の追跡変異⑨〜⑪が FAIL させる）。合成漏れは §0-F-2 の selftest ⑬ が機械検出する。**prep.json へのツール定数の追加記録は引き続き禁止**（§0-D-3・§0-E-1 — 本改訂は prep.json スキーマ・生成規則を変えず、検証 spec の合成側のみを是正する）。

### 0-F-2. Phase 3 selftest ⑬（検査8 再生成spec合成の網羅照合・新設）

**（改訂19: 帰属表へ swan の `logoTransplant`・`oldLogoRemoval` を追加し、決定的変異は⑬⑭を加えた計14件 — §0-G-4）**

v2 指示書の Phase 3 selftest へ次の1項目を追加する（①〜⑫は §0-E-10・v2 §3 のとおり不変）。

**⑬ 検査8 再生成spec合成の網羅照合（全8種）**:

- ツールは**チャーム単位の帰属表（field attribution map — 全8種 × `CHARM_SPECS[key]` の own フィールド名 → §0-F-1 の実測入力層／ツール定数層のどちらか一方）を単一の定数**として持つ（swan の threadColor・stitchMode だけが実測入力層で、他7種の同名フィールドはツール定数層、という**チャーム別の例外を表単位で一意に表現**する。1フィールドが両層に属する記載は帰属表の整合エラーとして FAIL）。**帰属表は CHARM_SPECS から動的に導出せず、CHARM_SPECS とは独立した明示的な凍結定数（全8種 × フィールド名の静的列挙）として定義する**（動的導出は (i) の完全性検査を恒真化するため禁止）
- **帰属表エントリの固定 projection**: 帰属表の各エントリは `{field（CHARM_SPECS[key] の own フィールド名）, 層, specPath（合成 spec 上の対応パス）, prepPath（prep 相当レコード上の対応パス — 対応記録を持たない純ツール定数は null）}` を持つ。specPath はトップレベルの同名フィールド。prepPath の全列挙: sizeMm → inputs.sizeMm・sizeMmMeasured → inputs.sizeMmMeasured・attachmentMode → inputs.attachmentMode・threadColor → inputs.threadColor・stitchMode → inputs.stitchMode・logoThreshold → pipeline.logoThreshold・calibration → inputs.calibration・stitchPathsWork → inputs.stitchPathsWork・grain → inputs.grain（**全 leaf** — jsonEqual の丸ごと比較で leaf 網羅を担保）・label → prep.label・source → prep.source.file（**正規化規則: CHARM_SPECS.source = "../" + prep.source.file の恒等で比較する** — 相対パス表現差は正規化してから一致判定）・sourceMode → inputs.sourceMode・toneMode → derived.toneMode（以上4件も1フィールド1エントリ・いずれも既存 assert の対象）・manualRecipe／logoRange／logoRoi／cropSource／beakRemoval／stoneRoi／stoneLumMax／**logoTransplant／oldLogoRemoval〔改訂19・§0-G-4〕** → null（純ツール定数）。**charmKey → key の一致は CHARM_SPECS の own フィールドではないため帰属表 projection に含めず、既存の独立した identity assert（prep.charmKey = key = 合成 spec.key）として扱う**。**(i)・(ii)・(ii)-b・own-property 双方向検査・undefined 禁止は、この projection に列挙された field／specPath／prepPath に限定して適用する**。**合成 spec 上の運用項目（key・logoRoiWork など CHARM_SPECS の own フィールドでないもの）は帰属表の対象外**であり、その検証は §0-F-1 の一致 assert ①〜④と §5-4 検査8 の既存照合が担う（帰属表へ CHARM_SPECS 外の項目を混在させない — 全体比較で正しい実装を FAIL させず、暗黙除外で合成漏れも見逃さないための範囲固定）
- **(i) 分類の完全性と排他性（per-charm）**: 全8種について `CHARM_SPECS[key]` の全 own フィールド名が帰属表の当該チャーム行に包含され、かつ各フィールドの層帰属が一意（実測入力層／ツール定数層のどちらか一方のみ）であること（**未分類フィールド・二重帰属の存在は FAIL** — 将来 CHARM_SPECS へフィールドを追加した際の合成漏れを機械検出する）
- **(ii) ツール定数層の一致（per-charm・本番関数直呼び）**: 生成直後の prep 相当レコード（**書き出しと同一の生成関数の出力**）から**本番の合成関数（verificationSpecFromPrep 相当）そのもの**を呼んで合成した検証 spec について、**チャームごとに** CHARM_SPECS[key] の own フィールドのうちツール定数層に分類される全フィールドが CHARM_SPECS[key] と jsonEqual であること（**ネスト構造は jsonEqual が丸ごと照合**する。分類列挙に載っていても当該チャームに無いフィールドは対象外。swan の threadColor・stitchMode は実測入力層帰属のため対象外）。**照合の写し実装を別に作らない**（本番の合成関数そのものを検査対象にする）。この per-charm 照合により、既分類のフィールド名を別チャームへ新たに追加した場合（例: `cropSource` を frenchie 以外へ追加）も、当該チャームの照合が合成漏れを検出する
- **(ii)-b 実測入力層の構成一致（per-charm）**: 同じ合成 spec について、帰属表で実測入力層に分類される spec フィールド（calibration・stitchPathsWork・grain・swan の stitchMode／threadColor）が、prep 相当レコードの対応記録値と jsonEqual であること（**実測入力層フィールドを prep から構成しない実装・ツール定数で上書きする実装を検出する**）
- **own-property の双方向検査（projection 限定）**: (i)・(ii)・(ii)-b の照合では、**帰属表 projection の各エントリについて、合成 spec 側 specPath の own-property 存在と、prep 相当レコード側 prepPath の own-property 存在（prepPath ≠ null のエントリのみ）を双方向に検査**する（projection にあるフィールドが合成 spec に存在しない／`CHARM_SPECS[key]` の own フィールドが projection に無い〔= (i) の完全性〕／実測入力層エントリの prep 側対応記録の欠落 — のいずれも FAIL。**undefined どうしの一致比較で通過させない**。projection 対象外の運用項目は本検査の対象にしない）
- **swan の検証入力（Phase 3 専用の決定的 fixture）**: swan の stitchMode・threadColor・stitchPathsWork は Phase 4 の実画素判定まで確定せず、CHARM_SPECS リテラル初期値 null は検証 spec で禁止されているため、selftest ⑬ の swan の prep 相当レコードは **§0-D-4 の判定注入機構（763cf1e の URL 指定）で detect 固定ベクトル `{stitchMode: "detect", threadColor: "light", stitchPathsWork: []}`（rev16 Phase 4 assetCommit `e71a1d80` の swan prep.json 実績値）を注入し、他7種と同一の生成経路から作成する**。fixture の判定値と PASS は prep-selftest.json へ記録する。この fixture は selftest の決定性のための検証入力であり、**Phase 4 の実判定（実画素で確定・§0-D-4・検査5）を先取り・拘束しない**。null は (iii) ⑧ の負変異専用であり正常系 fixture に用いてはならず、Phase 4 実成果物・過去成果物のファイルにも依存しない
- **(iii) 実効性回帰（12件 — ①〜⑧・⑫は負変異〔契約違反の検出〕・⑨〜⑪は追跡変異〔prep 実効値の反映確認〕。**対象チャームは決定性のため下記に固定する**）**: ①〜⑧は次の変異がいずれも FAIL として検出されること（不一致を検出できなければ selftest 不合格）: **①** prep 相当レコードの `inputs.sizeMm.w` を +1（**horse** — §0-F-1 新設①の回帰） **②** `pipeline.crop.x` を +1（frenchie — 新設③〔cropSource 側〕の回帰） **③** 合成 spec から `logoRange` を削除（**horse** — (ii) 照合の検出力回帰） **④** 合成 spec から `stoneRoi` を削除（swan — swan 固有定数の代表） **⑤** 合成 spec の `manualRecipe.id` を別文字列へ改変（**horseshoe** — 専用レシピ・ネスト構造照合の代表） **⑥** prep 相当レコードの `pipeline.logoRoiWork` の第1成分を +0.01（**cat** — 新設④〔logoRoi↔logoRoiWork 恒等一致〕の回帰） **⑦** `pipeline.crop.x` を +1（**dachshund** — cropSource を持たないチャームの新設③〔全面クロップ規範〕の回帰） **⑧** 合成 spec の swan `stitchMode` を CHARM_SPECS リテラル初期値（null）へ改変（(ii)-b 照合の回帰 — 実効値が prep 記録から構成されることの検証） **⑨** prep 相当レコードの `inputs.calibration.reason` へ固定 suffix「(selftest-mutation)」を追加（osanpo — manual-line 契約の代表。record 検証を通過する摂動） **⑩** `inputs.grain.calf.grainBase` を +1（**toypoodle** — grain leaf の代表。200 → 201 は検査F-2 の範囲検証〔1〜255・grainBase×(1+grainAmp)≤255〕を通過する） **⑪** `inputs.stitchPathsWork` へ固定ダミーパス1本 **`{points: [[100, 100], [200, 200]], widthMm: 1.0, dashMm: 2.0, gapMm: 2.0}`** を追加（**horse** — 頂点2点・作業座標・workSize 内で record 検証を通過し、CHARM_SPECS 初期値 [] と確実に異なる値）。**⑨〜⑪は追跡変異であり、合成 spec の当該フィールドが変異後の prep 記録値と jsonEqual であることを合格条件とする**（合成器が CHARM_SPECS の静的 seed／default 値を返す実装なら不一致になり FAIL — 実測入力層が prep 由来で構成されることの実証） **⑫** prep 相当レコードの horseshoe `inputs.threadColor` を "light" から "dark" へ改変（負変異 — CHARM_SPECS は不変のまま、新設②〔threadColor・stitchMode の swan を除く7種一致 assert〕が FAIL することの回帰。**通常生成では prep 値と CHARM_SPECS 値が一致するため、この変異が prep 側からの誤採用〔誤配線〕の検出力を実証する**）

### 0-F-3. toolVersion / schemaVersion

**`toolVersion` 0.8.0 → 0.9.0**（検証 spec 合成の是正・一致 assert 4点〔重複一致①〜③＋logoRoi↔logoRoiWork 恒等一致④〕・selftest ⑬ — 検査実装の変更に伴う増分・straight marker 連動・§5-4）。

**`schemaVersion` は 6 のまま不変** — prep.json のフィールド構成・生成規則（パイプライン①〜⑧'・検査1〜7・M・F・inspections17 の判定式・期待出力ハッシュの対象）はいずれも変わらないため、§5-4 の増分規則（スキーマ拡張または生成規則の変更）に該当しない。**ツール内 selftest の固定期待値（schemaVersion／toolVersion の照合 — 改訂17実装の旧固定値は toolVersion "0.8.0"）も 0.9.0 へ連動更新し、prep-selftest.json と phase-3-handoff.json へ toolVersion 0.9.0 の PASS 結果を記録する**（期待値の更新漏れは selftest の固定比較が FAIL するため機械検出される）。straight marker は toolVersion 連動のため、**Phase 4 は全8種を toolVersion 0.9.0 で再生成・再書き出しし、0.8.0 以前の出力（prep.json・派生 PNG・manual レイヤ）との混在を残さない**（prep.toolVersion／straight marker の照合が旧出力を機械的に排除する）。生成規則が不変のためデコード後 RGBA 画素は rev17 Phase 4 実測と同一になる見込みだが、**rev17 出力は未コミット（検査8 FAIL で停止）で比較正本が存在しないため、rev17 実測との同一性照合は Phase 4 の合格条件にしない**（生成規則不変の保証は selftest ①〜⑫ の生成規則回帰一式・検査1〜7・M・F・検査8 ③〔書き出し済み一式との全画素完全一致〕で担保する）。straight marker の toolVersion 文字列が変わるため PNG バイト列・byteLength・prep.json 記録値は 0.9.0 系で新規に確定する。

### 0-F-4. 報告先・ブランチ・再実施範囲

**報告先を `reports/charm-combo/rev18/` へ切替**（同期計画 §12-1 手順1）。**今回実施する Phase 3〜7 の新規成果物**（evidence・handoff・検査レポート）は `reports/charm-combo/rev18/` 配下へ固定し、**rev14〜17 配下は過去改訂の証跡として新規追記・変更を禁止する（読み取り専用の参照は可 — とくに rev14 配下の Phase 1 成果物は現行契約が参照する歴史的親成果物であり、rev18 配下への複製・移動もしない）**。Codex 専用 worktree のブランチ名は **`feature/charm-combo-codex-rev18`**（v2 §1）。

rev17 の Phase 3 成果物（toolCommit `0c5dbb5`・phase-3 evidence `cc5ce2b`）は**失効**とし、その内容（INPUT_MANIFEST・CHARM_SPECS・レシピ定数・§0-D〜§0-E の全契約転記・763cf1e 取り込み分）は新 Phase 3 で正規に取り込み直す（supersession.json に記録）。rev17 の Phase 4 以降は成果物 commit 未作成（検査8 FAIL で停止・assetCommit なし）のため失効対象の commit はない。**Phase 4 は全8種を toolVersion 0.9.0 で再生成**し、検査1〜7・M・F → 書き出し → 検査8（三者照合 — ③再生成照合まで §0-F-1 の合成契約で全8種 PASS）を再実行する。**Phase 5 形状 QA の全8種再承認要件（§0-E-11）は不変のまま継承する**（rev17 Phase 5 は未実施のため全8種の承認を新規に行う）。Phase 6 の完了条件（§0-E-11 の5条件）・Phase 7（192組・§0-A）も不変。

## 0-G. 改訂19の変更点一覧（2026-08-12）

**契機 — Codex変更還流票 `9a9fcac`（受領検証 PASS・改訂として初の還流票ありケース）**: rev18 の Phase 3〜4 完了後（toolCommit `b028f47` → phase-3 evidence `f598dbf` → assetCommit `e48dc41` → phase-4 evidence = implementationCommit `cf69bf0`・検査1〜8 は全8種 PASS）、ユーザー目視で「**swan の「&.」が cat ほど明瞭に見えない**」ことを確認し、Codex が還流票 `reports/charm-combo/rev18/change-tickets/20260812T060729Z-swan-logo-01.json`（changeTicketCommit `9a9fcac`・ticketPayloadSha256 `109e7ad90b8ce2e6c0f0a22b3d3deb1e7eb73794c3525bc4a04ccfed37f45f74`・visual-comparison sha256 `3611baead77e61556c47bf95a4b5c62923e30f44344bcc81b835070d54ea3c4a`）を提出して停止した。受領検証（2026-08-12）は §12 の要件どおり payload SHA・selectedLockCommit `37414ad`・originPhase Phase 4・implementationCommit・changedArtifacts before SHA ×4・inputBefore・一本道（`7ebbee9` → `b028f47` → `f598dbf` → `e48dc41` → `cf69bf0` → `9a9fcac`）を全一致で確認した。**payload SHA の検証方式 = 本フィールドを除外した canonical JSON に末尾 LF 1バイトを付加したバイト列の SHA-256**（§2-4 の canonical 規則〔末尾改行なし〕とは末尾改行1個の差があるが、決定的に再検証可能であることを確認して受領した。還流票の payload SHA はこの方式を正とし、supersession.json の changeTicketPayloadSha256 も同方式で照合する — lock manifest 等の §6-5 系 canonical JSON は従来どおり末尾改行なしで不変。**本読み替えは §8 の override map に固定行として記録する**）。

**原因**: swan のロゴは銀の細い筆記体「&」（logoRoi 内・BT.709 輝度 < 0.965・keep 除外の実画素 **1,410px**）だが、`detectLogo`（threshold 10）はスコア閾値検出のため **898px / 5成分（実画素の 64%）しか G 所有にできず**、リカラー後に輪郭が痩せる（cat は金の塗り活字で実画素 2,542px 中 2,086px = 82% を検出・2成分 — 見た目の差は検出率と字形〔筆記体 vs 活字〕の両方に由来し、閾値調整では解消しない）。

**意匠決定（2026-08-12・ユーザー目視選択）**: 抽出系4案（現行 898px／銀画素 1,410px／closing 1,494px／全開口部塗り 1,762px〔筆記体の字形が潰れるため不採用〕）の比較画像を経て、ユーザーが「**cat のロゴをそのまま swan に適用**」を要望。転写モック（cat 実寸 4.36mm／swan 現行寸 5.53mm／「&.」全体）の目視で **F2 = swan の現行ロゴ幅 5.53mm を維持した cat「&」活字字形の転写**を選択した。前提の了承事項: ①旧筆記体「&」は革面から除去する（§0-G-1） ②**「.」は転写しない — swan の「&.」の「.」の役割は実在のストーン〔keep・1,515px〕が果たす**（実測で swan 底部に独立したドット刻印は存在しない。cat の「.」相対位置へ転写するとストーン位置に重なることをモックで確認済み） ③実物の刻印は筆記体のまま、プレビュー表示のみ活字になる ④角度・位置（旧「&」bbox 中心 [407, 929]）・比率（現行ロゴ幅）・logoRoi・stoneRoi・sizeMm・calibration・crop（全面）は不変 — 還流票の「角度・位置・比率は変更しない」の充足。

**測定系**: 本節の数値はすべて実装忠実ハーネス実測（2026-08-12・PIL bilinear 縮小の作業座標 797×1200・§0-E-8 と同系）。測定系の忠実性は、現行 `detectLogo` 移植が 898px / 5成分 bbox(383,903) 49×52・cat 2,086px / 2成分・ストーン 1,515px bbox(430,924,478,966) を再現し **§0-E-8 記録値と完全一致**することで検証済み。**本 §0-G の bbox 表記はすべて [minX, minY, maxX, maxY]・両端包含（inclusive）の作業座標である**（§0-E-8 の記録と同規約。ROI 画素化の半開区間 x[x0, x1)×y[y0, y1) とは別物 — 除去領域・配置後・finalExpect・sourceExpect の全 bbox に適用）。

### 0-G-1. 旧筆記体ロゴの除去 `SWAN_OLD_LOGO_REMOVAL`（新設）

転写だけでは旧筆記体「&」の銀画素が革面（R 所有）に残り、リカラー後も陰影として視認される（転写字形で覆われない画素が 2,165 − 888 = 1,277px）。§0-E-6 のくちばし除去と同系の「決定的除去＋差分 range 検査」を新設する。

- **対象画素（一意定義）**: swan `logoRoi` [.4630, .7412, .5590, .8083] の画素化域 x[369,446)×y[889,970)（§0-E-1 の ROI 画素化規則）内で、**BT.709 輝度 < 0.965 かつ keep（ストーン・くちばし）でない全画素**（実測 **1,410px** = 筆記体「&」全体。ROI 内の非 keep 成分はこれのみ）。**輝度の判定ラスタ = ⑤-0.5 実行時点の work 画像 base.RGB**（①縮小〜④defringe 適用後。ロゴ領域は α = 255 の前景内部で ②〜④ が RGB を変更しないため縮小直後の RGB と同値 — §0-G 前文の測定系〔canonicalSource の bilinear 縮小画像・実測 1,410px〕と一致する）。
- **除去領域** = 対象画素 ∪ **チェビシェフ半径 2 の膨張**（アンチエイリアス縁の回収）から keep 画素を除いた集合 = 実測 **2,165px**・bbox (381,901,433,957)。
- **置換**: work 画像の base.RGB 上で除去領域を**白 (255,255,255) へ置換**する（**RGB のみの置換 — α・masks・keepMask は変更しない**）。工程順は **§5-4 パイプラインの ⑤-0（keepMask 確定）の直後・⑤（自動所有権候補）の前 = ⑤-0.5** とする（`COLORLESS_CLOSED_WHITE` は ③ の manual-alpha として適用済みであり、本除去は前景確定済みの革面内の RGB 置換である — くちばし除去〔α 背景化・§0-E-6〕とは方式が異なる）。置換後の画素は ⑤ の革候補判定・⑦' のグレイン焼き込み（shade 算出）で通常の革面画素として扱われ、⑧' を経て base.png へ反映される。**工程順は 除去（⑤-0.5）→ 転写（⑤・§0-G-2）に固定**する（転写 G 画素の下地 RGB も白置換済みとなる）。
- **検査**: **置換画素数 = 白置換を適用した除去領域の画素数（集合の cardinality・実測 2,165。置換前から RGB が白 (255,255,255) だった画素も含む — RGB 実変化画素数ではない）**を `removalExpectedDiffRange` **{countMin: 1732, countMax: 2598}**（実測 2,165 の ±20%・round）で照合し、範囲外は生成エラー（fail-fast）。同じ数え方の値を prep.json の `derived.oldLogoRemovalDiffPxCount` へ記録する（§0-G-5）。

**`SWAN_OLD_LOGO_REMOVAL` は次の9フィールドの凍結オブジェクト（`Object.freeze`）としてツールへ実装する**（§0-E-6 `BEAK_OUTLINE_REMOVAL` と同系の定数化 — 上記 prose 定義の機械表現。selftest は全フィールドの jsonEqual と removalExpect の完全一致を照合する〔§0-G-4 新回帰③⑥〕）:

| field | 値 | 規定 |
|---|---|---|
| id | "swan-old-logo-removal-20260812-v1" | |
| roi | [.4630, .7412, .5590, .8083] | **CHARM_SPECS.swan.logoRoi との同値 assert**（値の焼き込み＋恒等照合 — §0-F-1 一致 assert④と同系）。画素化 x[369,446)×y[889,970) |
| lumMax | .965 | BT.709 輝度 < lumMax の画素が対象（¬keep） |
| dilationMetric | "chebyshev" | 膨張の距離規則 |
| dilationRadiusPx | 2 | 膨張半径 |
| keepExclusion | true | 対象抽出・膨張後の両段階で keep 画素を除外 |
| replacementRgb | [255, 255, 255] | base.RGB の置換値（α・masks・keepMask は不変） |
| removalExpect | { px: 2165, bbox: [381, 901, 433, 957] } | 除去領域（cardinality）の**完全一致 assert** |
| removalExpectedDiffRange | { countMin: 1732, countMax: 2598 } | prep 記録照合の fail-fast range（§0-G-5） |

### 0-G-2. cat 字形の転写 `SWAN_LOGO_TRANSPLANT`（新設）

swan の G 所有（箔）画素は `detectLogo` に代えて、**ツール定数 `glyphMask`（cat の活字「&」字形・本節末尾の正本ブロック）を anchorCenter へ配置して直接生成**する（horseshoe / osanpo の「manualRecipe が G 所有画素を直接生成する」系譜の転写型。**ツールの実行工程は glyphMask 定数から始まり、cat からの抽出・縮小は実行しない** — 縮小実装差の排除。生成由来は本節後段の provenance）。全フィールドはツール定数（`Object.freeze`）とし、次の値を正本とする。

| field | 値 | 規定 |
|---|---|---|
| id | "swan-logo-transplant-20260812-v1" | |
| glyphMask | 48行 × 49文字の "0"/"1" 文字列配列（**本節末尾の正本ブロック — 48行を一字一句そのまま転記**） | **実行時の字形正本**。行順 = y 0〜47・文字順 = x 0〜48・"1" = 転写画素・**popcount 980** |
| glyphMaskSha256 | `9e94d7e7ee1d738920f48685805a8ed2fa9ffa5ed4c634dacb38290c9899d7b2` | 48行を LF（U+000A）区切りで連結した UTF-8 バイト列（末尾 LF なし）の SHA-256。**selftest は構造（48行・各行49文字・"0"/"1" のみ・popcount 980）と本値の一致を assert する** |
| anchorCenter | [407, 929] | 旧「&」bbox (383,903,431,955) の中心。**配置原点 = (anchorCenter.x − floor(49/2), anchorCenter.y − floor(48/2)) = (383, 905)**・配置後 bbox (383,905,431,952) |
| keepExclusion | true | 配置後、keep（ストーン等）画素を除外（実測の削れ 7px — ストーン bbox x0=430 との近接部） |
| **finalExpect（最終 G 所有の期待値）** | **{ px: 973, componentCount: 1, bbox: [383, 905, 430, 952] }** | 生成結果の完全一致 assert。⑤ の G 所有画素として以後のパイプライン（masks・manual-owner・metalTone リカラー）は既存契約どおり |
| transplantRange | { countMin: 778, countMax: 1168, componentMin: 1, componentMax: 1 } | 973 の ±20%・round。成分数は決定的生成のため 1 固定 |

**ツール定数 `SWAN_LOGO_TRANSPLANT` のフィールドは上表の7点（id・glyphMask・glyphMaskSha256・anchorCenter・keepExclusion・finalExpect・transplantRange）で全て**であり、下記 provenance のフィールド（sourceCharm・sourceSha256・sourceRoi・extractLumMax・sourceExpect・targetSizePx・binarizeThreshold）は**ツール定数に含めない**（記録は本節の記述のみ — selftest ⑬ の own-property 検査・(ii) jsonEqual の対象は CHARM_SPECS 上の実フィールド7点に限る）。

**glyphMask の生成由来（provenance — 定数の導出記録。ツールの実行工程ではない〔cat からの再抽出・再測定は行わない — bilinear 縮小の実装差を排除するためツールは glyphMask 定数から直接生成する。osanpo `stitchRange` の「基準実装値は参照値であり合格条件ではない」〔§0-E-4〕と同型の扱い〕。**ただし provenance 定数値の転記照合〔全7フィールド jsonEqual — §0-G-4 ⑤〕は Phase 3 selftest の合格条件である**）**: Phase 1 凍結済み cat canonicalSource（sourceCharm "cat"・sourceSha256 `563b5c975eaebbafa3fbb25a1720d0c5021022dca23cb2f069d46553be10bce7` = INPUT_MANIFEST.cat.sha256〔§4-1c〕）の cat 作業座標 1200×1200 上で、sourceRoi [.4875, .7092, .5900, .7958]（cat `logoRoi` と同値・§0-E-1 規則で画素化 → x[585,708)×y[851,955)）内の BT.709 輝度 < extractLumMax .965 の8近傍最大成分（= 活字「&」・基準実装値 sourceExpect { px: 2408, bbox: [599, 865, 675, 939] }〔77×75〕）を bbox で切り出し、**基準実装 = PIL bilinear**（§0-G 前文の測定系）で targetSizePx [49, 48]（= [49, round(75 × 49 / 77)]）の 0〜1 実数マスクへ縮小し、binarizeThreshold .5 以上を 1 に2値化して得た（2026-08-12 実測・980px）。**ツールは provenance を別の凍結定数 `SWAN_LOGO_TRANSPLANT_PROVENANCE`（フィールド: sourceCharm・sourceSha256・sourceRoi・extractLumMax・sourceExpect・targetSizePx・binarizeThreshold — 検証 spec 合成・帰属表の対象外）として保持し、provenance の転記照合（**全7フィールドが本節契約値と jsonEqual・特に `sourceSha256` = INPUT_MANIFEST.cat.sha256〔§4-1c が入力 SHA の正本 — CHARM_SPECS に sha256 own フィールドは存在しない〕の一致**。cat からの再抽出は実行しない）を Phase 3 selftest の独立項目として照合し、PASS 結果と値を Phase 4 証跡（prep-inspection-1-8.json）へ転記する（§0-G-4 新回帰⑤）**。

**glyphMask 正本（48行 × 49文字・上から y=0）**:

```
0000000000000000000011111111000000000000000000000
0000000000000000011111111111100000000000000000000
0000000000000001111111111111110000000000000000000
0000000000000011111111111111111000000000000000000
0000000000000111111110000011111100000000000000000
0000000000001111111100000001111100000000000000000
0000000000001111111000000001111100000000000000000
0000000000001111111000000000111100000000000000000
0000000000011111110000000000111110000000000000000
0000000000011111110000000000111100000000000000000
0000000000011111110000000001111100000000000000000
0000000000011111110000000001111100000000000000000
0000000000011111111000000001111100000000000000000
0000000000011111111000000011111000000000000000000
0000000000011111111000000111110000000000000000000
0000000000011111111100001111110000000000000000000
0000000000001111111100011111100000000000000000000
0000000000001111111111111110000011111111111111000
0000000000001111111111111110000111111111111111100
0000000000000111111111111000000011111111111111100
0000000000000111111111100000000001111111111000000
0000000000000111111111100000000000111111100000000
0000000000111111111111100000000000111111100000000
0000000001111111111111110000000000111111000000000
0000000111111111111111111000000000111110000000000
0000001111111101111111111000000000111110000000000
0000111111110000111111111100000001111100000000000
0000111111100000011111111100000001111100000000000
0001111111000000011111111110000011111000000000000
0011111110000000001111111110000111111000000000000
0111111110000000001111111111000111110000000000000
0111111110000000000111111111100111100000000000000
1111111100000000000111111111111111100000000000000
1111111100000000000011111111111111000000000000000
1111111100000000000001111111111110000000000000000
1111111100000000000000111111111110000000000000000
1111111110000000000000111111111100000000000000000
1111111110000000000000011111111110000000000000000
1111111111000000000000001111111111000000000000111
1111111111000000000000011111111111100000000001111
0111111111100000000000111111111111111000000001111
0111111111110000000001111111111111111100000011110
0111111111110000000111111101111111111111100111110
0011111111110000011111111000111111111111111111100
0001111111111111111111110000011111111111111111000
0000111111111111111111000000001111111111111110000
0000011111111111111100000000000011111111111100000
0000000001111111100000000000000000011111100000000
```

- `logoRange` の照合座標系は従来どおり作業座標（`logoPxCountWork` / `logoComponentCountWork`）。swan の照合値は本転写の生成結果に一致する。
- ストーンの判定規則（§0-E-8: `stoneRoi` 内 輝度 < `stoneLumMax` の最大成分 = keep 1,515px）・くちばし除去（§0-E-6）・革面レシピ（§0-E-1）は**不変**。

### 0-G-3. CHARM_SPECS.swan の変更と §0-E-8 の関係

- `logoThreshold`: 10 → **null**（swan は `detectLogo` を用いない）。**`detectLogo` の適用対象は horse / frenchie / dachshund / toypoodle / cat の5種**となり、G 所有直接生成は horseshoe（manualRecipe）・osanpo（manualRecipe）・**swan（logoTransplant）**の3種。§0-E-8 の「6種」記述は本節が上書きする（§0-E-8 の keep 除外規則は **detectLogo を用いる現行5種（horse / frenchie / dachshund / toypoodle / cat）すべてに適用を維持**する（実効の keep 対象装飾を持つのは現行 cat のみだが、機構は5種共通 — §5-4 ⑤）。swan の除外実測回帰〔1,221→898〕のみ失効し §0-G-4 の新回帰に置換）。
- `logoRange`: {countMin: 718, countMax: 1078, componentMin: 3, componentMax: 8} → **{countMin: 778, countMax: 1168, componentMin: 1, componentMax: 1}**（転写生成 973px / 1成分の ±20% — swan の componentMax +3 特例〔銀線画の断片化対策〕は転写方式への移行に伴い廃止）。
- 新フィールド: `logoTransplant: SWAN_LOGO_TRANSPLANT`（§0-G-2）・`oldLogoRemoval: SWAN_OLD_LOGO_REMOVAL`（§0-G-1）。
- **不変**: `logoRoi`（除去対象の定義域・転写の検証域として維持）・`stoneRoi`・`stoneLumMax`・`beakRemoval`・`manualRecipe`（革面 `closedWhiteRecipeFor("swan")`）・`calibration`・`sizeMm` 90×90・`sizeMmMeasured`・`attachmentMode`・`threadColor`・`stitchMode`・crop（全面）・grain・**label・source・sourceMode・toneMode・stitchPathsWork**。**この列挙で CHARM_SPECS.swan の own フィールドは全て網羅されており、改訂19 で値が変わるのは `logoThreshold`・`logoRange` の2点、新設は `logoTransplant`・`oldLogoRemoval` の2点、それ以外はすべて不変である**（selftest ⑬ (i) の帰属表完全性検査が own フィールドの網羅を機械保証する — §0-F-2・§0-G-4）。

### 0-G-4. selftest ⑬ の拡張と swan 回帰の置換

- **帰属表（§0-F-2）**: swan のエントリへ `logoTransplant` → null・`oldLogoRemoval` → null（いずれも**ツール定数層**・prepPath null〔純ツール定数〕）の2件を追加する（1フィールド1エントリ・own-property 双方向検査の対象）。**§0-F-1 (b) 表と §0-F-2 の prepPath 全列挙にも同2フィールドを追記済み（本改訂による更新 — 純ツール定数 → null）**。他7種の帰属表は不変。
- **決定的変異**: §0-F-2 の12件へ次の2件を追加し**計14件**とする。**⑬** 合成 spec 側の swan `logoTransplant.anchorCenter` を [407, 929] → [417, 929] へ改変（負変異 — ツール定数層 per-charm jsonEqual〔selftest ⑬ (ii)〕が FAIL することの回帰。転写位置の誤配線検出） **⑭** 合成 spec 側の swan `oldLogoRemoval` フィールドを削除（負変異 — 帰属表の完全性検査〔(i)〕の own-property 双方向検査または (ii) が FAIL することの回帰）。
- **Phase 3 selftest の swan 回帰の置換**: 旧「keep 除外の有無で 1,221px/6成分 → 898px/5成分」（§0-E-8・detectLogo 前提）は**失効**。新回帰 = ①転写の生成が **973px / 1成分・bbox (383,905,430,952)** に一致 ②**glyphMask の構造検証**（48行・各行49文字・"0"/"1" のみ・popcount 980・`glyphMaskSha256` 一致 — `sourceExpect` は生成由来の記録であり実行時の抽出検証は行わない〔定数転記照合⑤の対象ではある — §0-G-2〕） ③除去領域が **2,165px** で `removalExpectedDiffRange` 内 ④ストーン keep 1,515px 不変 ⑤**provenance 転記照合** — `SWAN_LOGO_TRANSPLANT_PROVENANCE` の全7フィールド（sourceCharm・sourceSha256・sourceRoi・extractLumMax・sourceExpect・targetSizePx・binarizeThreshold）が §0-G-2 の契約値と jsonEqual、特に `sourceSha256` が **INPUT_MANIFEST.cat.sha256**（= §4-1c の cat 正規入力 SHA・freeze JSON 由来 — CHARM_SPECS に sha256 own フィールドは存在しない）と一致すること（provenance-only assert・独立項目・cat からの再抽出は実行しない・検証 spec 合成の対象外 — §0-G-2） ⑥**定数転記照合** — `SWAN_LOGO_TRANSPLANT`（7フィールド・§0-G-2 の表）・`SWAN_OLD_LOGO_REMOVAL`（9フィールド・§0-G-1 の表）の全フィールドが本計画書の契約値と jsonEqual であること ⑦**transplantRange 独立範囲判定** — 生成画素数・成分数が `transplantRange` 内であること（`finalExpect` 完全一致とは別の独立ゲート）および CHARM_SPECS.swan.logoRange = transplantRange の同値 assert。
- §0-F-1 の再生成 spec 合成契約・投影規則・一致 assert 4点は**不変**（`logoTransplant`・`oldLogoRemoval` はツール定数層に属し、検証 spec への合成対象に**含める**。**実装上の注意: 現行ツールの合成関数〔verificationSpecFromPrep 相当〕は帰属表から動的生成しない手動オブジェクト構築であるため、改訂19 の実装では両フィールドの明示投影を合成関数へ追加すること** — 追加漏れは selftest ⑬ の正常系 own-property 検査〔(i)(ii)〕・変異⑭・検査8 ③再生成照合が FAIL として検出する。検査8 ③再生成照合は転写・除去を含むパイプラインを独立再実行して RGBA 完全一致を照合する）。

### 0-G-5. toolVersion / schemaVersion

- **toolVersion 0.9.0 → 0.10.0**・**schemaVersion 6 → 7**（swan の G 所有生成規則の変更〔detectLogo → 転写〕＋旧ロゴ除去による出力画素値の変更＋prep.json への検査記録フィールド追加 — swan の `derived` へ `oldLogoRemovalDiffPxCount`（実測置換画素数）・`logoTransplantPxCount`（keep 除外後の生成画素数）を追加し、`inspections17.logoInspection.logoThreshold` は null を記録〔horseshoe / osanpo と同型〕）。ツール内 selftest の固定期待値（改訂18実装の旧固定値 0.9.0）も 0.10.0 へ連動更新し、straight marker の toolVersion も連動する。**全8種を v7 の生成規則で再書き出し**、v6 以前の prep.json・派生アセットとの混在を残さない（生成規則が変わるのは swan のみだが、schemaVersion の一意性のため全種を再書き出しする）。prep-selftest.json と phase-3-handoff.json へ 0.10.0 の PASS 結果を記録する。
- RGBA 同一性照合について: swan は生成規則が変わるため rev18 出力と一致しない（比較対象外）。**他7種は生成規則が不変であり、rev18 assetCommit `e48dc41` の書き出し済み全 PNG（`assets/charms/<key>.base.png`・`<key>.masks.png`・実在する全 `<key>.manual-*.png`（§5-2 のフラット命名 — ディレクトリ分割はない。prep.json は比較対象外〔schemaVersion 7 で再確定〕））と RGBA が一致することを Phase 4 で確認し、不一致は生成エラーとする**（rev18 出力はコミット済みで比較正本が存在する — 改訂18 の「比較正本なし」条項は本改訂で解消）。**比較オラクルの役割分離**: `e48dc41` は §0-G-6 で失効した rev18 assetCommit だが、本比較では**読み取り専用の比較オラクル**としてのみ用いる（§12-3 再開ゲートの「旧 commit を Phase 入力にしない」原則との整合 — phaseInputCommits・carry-forward 対象には含めない）。Phase 4 evidence へ `comparisonOracleCommit`（e48dc41 のフル SHA）・比較対象 PNG のパス集合・オラクル側取得 SHA を記録する。

### 0-G-6. 報告先・ブランチ・再実施範囲

- **報告先は `reports/charm-combo/rev19/` へ固定**し、rev14〜18 配下は読み取り専用とする（還流票 `9a9fcac` の change-tickets 配下も追記・変更禁止）。Codex のブランチは `feature/charm-combo-codex-rev19`・起点は新 revision lock が選択する selectedPhase2EvidenceCommit。
- **失効**: rev18 toolCommit `b028f47`・phase-3 evidence `f598dbf`・assetCommit `e48dc41`・phase-4 evidence `cf69bf0`（= restartPhase 以降の rev18 成果物。監査証跡として履歴に保存・内容不変）。旧 lock `37414ad` は新 lock manifest の supersedesLockCommit で機械失効。Phase 5 以降は rev18 で未実施のため失効対象 commit はない。
- **還流票の derivedImpact 申告との差異の記録**: 還流票 `9a9fcac` の payload は `derivedImpact.base = false`（ロゴ層のみの想定申告）だが、採用した意匠は ⑤-0.5 の旧ロゴ除去（§0-G-1）が swan の base.RGB を変更するため、**実影響は base = true**（swan の material 別 base 全点・masks・prep.json が再生成対象）である。**還流票は immutable のため原本は改変せず、本節と新 revision lock の supersession.json の `derivedImpactOverride` フィールド（実影響の記録 — base = true・対象 = swan の全派生アセット）で申告を上書きする**。失効・再実施範囲は本節冒頭の列挙（rev18 Phase 3〜4 成果物の全失効）に既に含まれており、対象 commit の追加はない。
- **Phase 5 は swan の転写後「&.」の目視承認を必須とする**（還流票 visualApproval の status: requested の解消 — 正式な形状承認 ID をここで発行する。全8種の再承認要件は §0-E-11 から不変継承）。**承認の機械検証契約（改訂19）**: rev19 の shape-approval.json は `resolvedChangeTickets[]` を必須フィールドとして持ち、各要素は { `ticketId`: "20260812T060729Z-swan-logo-01"・`changeTicketCommit`: `9a9fcacf2094bddff9657d48bad4115cc405ffd1`・`changeTicketPayloadSha256`: `109e7ad90b8ce2e6c0f0a22b3d3deb1e7eb73794c3525bc4a04ccfed37f45f74`・`visualApproval`: "approved"・`approvalEvidenceId`（正式な形状承認 ID — 既存 AE-YYYYMMDD-NN 形式）・`scope`: "swan &.（glyphMask 転写＋ストーン）"・`referenceSha256`（**型 = { <リポジトリ相対パス>: <RGBA SHA-256> } のオブジェクト** — 承認時に参照した swan の全 PNG〔`assets/charms/swan.base.png`・`swan.masks.png`・`swan.manual-alpha.png`・`swan.manual-owner.png`・`swan.manual-keep.png`（§5-2 のフラット命名）〕を §5-4 の最終 PNG 独立デコード方式で列挙） } とする。**rev19 で解消すべき還流票の完全集合 = { "20260812T060729Z-swan-logo-01" } の1件（§12 確定記録が正本）**。**phase-5-handoff.json の phase6Ready は次の論理積から導出する**: ①全8種の通常形状承認（§0-E-11 継承の再承認要件）の完了 ∧ ②`resolvedChangeTickets[]` が上記完全集合と 1:1 対応（過不足・重複なし） ∧ ③各要素の**必須7フィールドすべて**の機械照合一致 — `ticketId`・`changeTicketCommit`・`changeTicketPayloadSha256` は §0-G 前文値と完全一致・`visualApproval` = "approved"・`approvalEvidenceId` は AE-YYYYMMDD-NN 形式かつ **同じ shape-approval.json 内の全8種形状承認記録（同期計画 §9-5 の許可成果物内で完結 — 別ファイルの承認台帳は要求しない）に同 ID のエントリが実在**・`scope` は本節の値と完全一致・`referenceSha256` は**列挙された各パスの実在ファイルを独立デコードした再計算 SHA と全一致**。**いずれか1つでも欠落・不一致・未解消の還流票が残る場合は phase6Ready = false とする**（同期計画 §9-5 の汎用承認記録への上書き — §8 override map の機構）。
- **数量・色・freeze JSON・sizeMm・他7種の生成規則はすべて不変**（planRevision のみ 18 → 19 増分・§0-A。counts.finiteCases 192 等の数量契約は不変）。
...
生成入力（canonicalSource・§4-1c） → ①回転・クロップ・縮小（長辺≤1200px） → ②自動背景マット（しきい値） → ③manual-alpha適用
     → ④defringe → **⑤-0 keepMask 確定〔改訂17 新設・§0-E-6/§0-E-8〕** → **⑤-0.5 swan 旧ロゴ除去〔改訂19 新設・§0-G-1 — swan のみ・base.RGB の白置換〕**
     → ⑤自動所有権候補（革/ロゴ/ステッチ。**keep 画素を除外して算出**。**swan の G 所有候補は detectLogo に代えて `SWAN_LOGO_TRANSPLANT` の glyphMask 配置で直接生成〔改訂19・§0-G-2〕**）
...
**この工程順の変更（改訂17）は生成規則の変更であり、`schemaVersion` 5 → 6〔改訂17 当時・§0-E-9〕の増分理由の一つであった**（現行の schemaVersion は改訂19 の **7** — §0-G-5。改訂19 の ⑤-0.5 旧ロゴ除去・glyphMask 転写も同様に生成規則の変更であり 6 → 7 増分の理由である）。
...
8. **再生成一致（三者照合）**: 準備ツールの再検証モードで、書き出し済みの canonicalSource＋prep.json＋manual-*.png を読み込み、次の3照合すべてに合格すること。①**入力照合**: 生成入力（canonicalSource）・manual-*.png の SHA-256 が prep.json 記載値と一致し、**生成入力・sourceMode は §4-1c の正規入力マニフェストとも一致**すること（検査M-1 と同一の照合 — prep.json の自己申告値どうしの整合だけでは通過できない。**material ごとの grain 入力 SHA〔マニフェスト照合込み〕を含む** — §5-2・§5-6） ②**出力照合**: 書き出し済み base（**material 別 base 全点**）/masks の**最終 PNG 独立デコード**（下記 straight PNG 契約）による RGBA ハッシュが prep.json の期待出力ハッシュと一致 ③**再生成照合**: パイプライン①〜⑧'（**⑦' の二段階〔⑦'-1 グレイン焼き込み＋⑦'-2 再defringe。colorless のみ〕**と⑧' の二段階〔⑧'-1 幾何変換＋⑧'-2 再defringe〕を含む・§5-2）を、**§0-F-1 の再生成spec合成契約で構成した検証 spec（prep.json の実測入力層 × CHARM_SPECS のツール定数層の合成・§0-F-1 の一致 assert 4点〔重複一致①〜③＋logoRoi↔logoRoiWork 恒等一致④〕込み — 改訂18）**により独立再実行した結果のデコードRGBA・派生定義値（calibration・keepPxCount・aspectCorrection・**materialAssets・grainScaleX / grainScaleY・pxPerMmWorkW / pxPerMmWorkH・flatWhite〔colorless — §5-2 の入力から再計算し記録値と完全一致させる〕**を含む prep.json 記録値一式）が、書き出し済みファイル・prep.json 記載値と完全一致（比較対象は常に「書き出し済み一式」なので初回生成でも循環しない）。合否は prep.json の `verify` フィールドに追記する。**改訂19 追加ゲート（§0-G-5）**: 検査8 と併せて、**swan を除く7種は rev18 assetCommit `e48dc41` の書き出し済み PNG との RGBA 完全一致（最終 PNG 独立デコード比較・対象 = `assets/charms/<key>.base.png`・`<key>.masks.png`・実在する全 `<key>.manual-*.png`（§5-2 のフラット命名 — ディレクトリ分割はない。prep.json は比較対象外〔schemaVersion 7 で再確定〕））を確認する（生成規則不変の回帰・不一致は生成エラー。swan は生成規則変更〔⑤-0.5 除去・glyphMask 転写〕のため比較対象外）**。**swan は RGBA 比較の代替として、本番パイプライン出力に対する次の必須 assert 5点を検査8 と併せて実行し、結果（判定・実測値・provenance 転記照合の PASS）を prep-inspection-1-8.json へ記録する（§0-G-2・§0-G-5）: ①G 所有が `finalExpect` { px: 973, componentCount: 1, bbox: [383, 905, 430, 952] } に完全一致 ②glyphMask 構造検証（48行・各行49文字・"0"/"1" のみ・popcount 980・`glyphMaskSha256` 一致） ③除去領域が `removalExpect` { px: 2165, bbox: [381, 901, 433, 957] } に完全一致し（`derived.oldLogoRemovalDiffPxCount` の記録・bbox 照合込み）`removalExpectedDiffRange` 内 ④ストーン keep 1,515px 完全一致 ⑤`derived.logoTransplantPxCount` と成分数が `transplantRange` 内（`inspections17.logoInspection.rangeCheck` への記録と一致 — finalExpect 完全一致とは別の独立ゲート）**
...
- `schemaVersion`（**スキーマ拡張または生成規則の変更時に増分する** — フィールド構成が不変でも、同一 schemaVersion に生成規則違いの prep.json が混在すると検査8の再生成一致が破れるため規則変更も増分対象。aspectCorrection・sizeMmMeasured を導入した改訂10、⑧' を二段階化〔再defringe〕した改訂12、無着色素材モードを導入した改訂13、**全8種 colorless 化・material-aware 化〔materialAssets・grain per material〕・straight PNG 契約化の改訂14**、**⑦' の出力画素値を変える生成規則の変更〔grainSourceInset・shadeMin〕と α／masks／keep を変える生成規則の変更〔COLORLESS_CLOSED_WHITE・outlineCaptureRadius・BEAK_OUTLINE_REMOVAL〕、および grain 記録項目の拡張〔grainSourceInset・effectiveWidth・effectiveHeight・effectiveRoiSha256・shadeMin〕を行った改訂17**でそれぞれ増分 — **改訂14〜16 の schemaVersion は 5（改訂16では不変 — §0-D-5）、改訂17で 5 → 6 へ増分（§0-E-9・改訂18では不変 — §0-F-3）、**改訂19で 6 → 7 へ増分（§0-G-5 — swan の G 所有生成規則の変更〔detectLogo → cat 字形転写〕・旧ロゴ除去・検査記録フィールド追加）**・toolVersion は改訂16で 0.6.0 → 0.7.0、改訂17で 0.7.0 → 0.8.0、改訂18で 0.8.0 → 0.9.0〔検査実装のみの変更・§0-F-3〕、**改訂19で 0.9.0 → 0.10.0〔§0-G-5〕**（straight marker の toolVersion も連動）。**全8種を v7 の生成規則で再生成し、v6 以前の prep.json・派生アセット・旧 manual レイヤ〔座標系が旧素材由来のもの／旧生成規則由来のもの〕との混在を残さない**〕、ツールバージョン
...
- **改訂19 追加（§0-G-5）**: swan の `derived` へ `oldLogoRemovalDiffPxCount`（非負整数 — **⑤-0.5 の除去領域 cardinality〔§0-G-1 の数え方 — 置換前から白だった画素も含む〕**・`removalExpectedDiffRange` 照合）・`logoTransplantPxCount`（非負整数 — keep 除外後の転写生成画素数・`transplantRange` 照合）を追加する（**いずれも作業座標〔work・797×1200〕の値 — `logoPxCountWork` と同系。アセット座標の対応記録は持たず、範囲判定・inspections17 記録もこの work 値に対して行う**）。**この2項目の追加と swan の生成規則変更（⑤-0.5 旧ロゴ除去・glyphMask 転写）が `schemaVersion` 6 → 7 の増分理由である**（§0-G-5 — 全8種を v7 で再書き出し）
...
**技術値の優先関係（override map — 同期計画に残る旧色契約の再流入防止）**: 同期計画の権限は**進行機構（Phase 順序・commit chain・handoff・専用 worktree・還流票）に限定**し、**技術値（素材・色・検査・数量）は本計画書 revision 19 が優先**する。特に同期計画の次の記述は本計画書の現行契約へ読み替える（同期計画自体は Phase 1 完了時点の文書として不変のまま保存する）:

| 同期計画の記述 | 読み替え（本計画書 rev19） |
|---|---|
| §6-5 の revision lock 検証アルゴリズム（15手順）および §2-4 の成果物 path 契約が固定している `reports/charm-combo/rev14/…`（plan-review-{arch,diff,cross-check}.json・plan-review.json・revision-lock-verification.json・phase-2-handoff.json 等）と、同 §6-5 の `counts.finiteCases = 216` | **`reports/charm-combo/rev19/…` と `counts.finiteCases = 192` へ読み替える**（§12-1 手順1 が「報告先を `reports/charm-combo/revNN/` へ切り替える」と規定しており、rev14 は改訂14 当時の具体値。**path の revNN 部分と finiteCases 以外の検証手順・条件式・親子鎖の要件は一切変更しない**）。改訂19 の具体値は revNN = **rev19**・finiteCases = **192**・planRevision = **19** |
| §6-5 の manifest 必須フィールド表「planRevision \| 14」・検証アルゴリズム step 8 の抽出対象「改訂14計画とv2指示書の2文書から、改訂14、…、216有限値、…を独立抽出する」・step 10 の旧指示書失効メタデータ期待値（rev14 当時の successorPlan 等）・§2-1 の「planRevision は作成予定の14を記録する」 | **planRevision の固定値はすべて現行 19 へ読み替える**。step 8 は「**改訂19 計画と v2 指示書〔改訂19 対応〕の2文書から、改訂19・8種・2 material profile・27 leatherVariant・192有限値・81配置・CHARMS 10項目・variant 8項目・material 6項目を独立抽出する**」へ、step 10 の期待値は「**documentCommit に固定された旧指示書の現行失効メタデータ（status: superseded・successorPlan ＝ 本計画書 revision 19・successorInstructions ＝ v2 指示書〔改訂19 対応〕・lockManifest ＝ plans/charm-combo-revision-lock.json・supersededAt は記録値）との完全一致**」へ読み替える（§12-1 手順1「planRevision を必ず1増分して報告先を revNN へ切り替える」の帰結を検証手順の期待値まで貫徹するもの。**rev19 の revision lock manifest は planRevision ＝ 19・counts.finiteCases ＝ 192〔§0-A〕であり、lock 検証・revision-lock-verification・phase-2-handoff はこの読み替え後の期待値で生成・照合する** — rev15 lock〔planRevision 15〕・rev16 lock〔5cc72dd・planRevision 16〕・rev17 lock〔4df10c2・planRevision 17〕・rev18 lock〔37414ad・planRevision 18〕も同一の読み替え規則で検証済みの前例） |
...
| 同期計画 §7〜§11 の Phase 4〜7 開始時検証・成果物要件が参照する「plans/charm-combo-display-plan.md revision 14」「revision 14計画とv2指示書」（同期計画 769・884・961・1049 行ほか各所） | **「本計画書 revision 19 と v2 指示書〔改訂19 対応〕」へ読み替える**（§12-1 手順1 の帰結 — 各 Phase の開始時検証・数量照合・成果物要件はすべて rev19 の正本 documents 3点〔lock の documents と同一 SHA〕を参照する。**例外 = Phase 1 ancestry の rev14 参照〔reports/charm-combo/rev14/ の Phase 1 証跡・phase-1-approvals.md・sourceFreezeCommit 8b3bdb4 の凍結成果物・phase-1-handoff〕は歴史的事実であり読み替えない** — Phase 1 は carry-forward・§0-E 前文） |
...
| §6-2「mutation試験」が列挙する**5種**（Float32化・mirror誤り・loop-order変更・grain map全1・非革画素変更） | **6種へ拡張**（改訂17 — `shadeMin` 適用範囲誤りを追加。完全定義は本計画書 §5-4・根拠は §0-E-3）。同期計画側の列挙は5種のまま残るが、**技術値の正本は本計画書 rev19** であり、Phase 4 は6種すべての検出をもって検査F合格とする |
...
| §12-1 の変更還流票・supersession 記録が参照する `changeTicketPayloadSha256`／還流票 `ticketPayloadSha256` の canonical JSON 規則（§2-4 — 末尾改行なし） | **ticket 系ハッシュのみ「§2-4 canonical JSON ＋ 末尾 LF 1バイト」のバイト列の SHA-256 で照合する（改訂19 §0-G 前文 — 還流票 `9a9fcac` の受領検証で確認した実績方式）。§6-5 系（lock manifest・phase-2-handoff・supersessionPayloadSha256 等）は従来どおり末尾改行なしで不変** |

| 同期計画 Phase | 旧フェーズ対応 | 内容（進行・handoff 詳細は同期計画の当該節） |
|---|---|---|
| Phase 3 ツール同期 | （新設） | `tools/charm-prep.html` を改訂19契約へ更新: INPUT_MANIFEST 8種（§4-1c）・COLOR_REFERENCE_MANIFEST・LEATHER_VARIANTS 27・MATERIAL_PROFILES 2・**schemaVersion 7**〔§0-G-5〕・**toolVersion 0.10.0**〔§0-G-5〕・swan の CHARM_SPECS 登録・改訂16分（frenchie cropSource 定数〔§0-D-2〕・osanpo レシピ・sizeMmMeasured 8種 true〔§0-D-1〕・763cf1e の swan ステッチ判定 UI／selftest 復元堅牢化）に加え、**改訂17分 = `COLORLESS_CLOSED_WHITE` レシピ新設と6種への適用〔§0-E-1〕・`grainSourceInset: 2`〔§0-E-2〕・⑦' の `shadeMin: 0.78`〔§0-E-3〕・osanpo レシピ v2〔§0-E-4〕・horseshoe の `outlineCaptureRadius: 12`／`outlineLumMax: 0.75`〔§0-E-5〕・swan の `BEAK_OUTLINE_REMOVAL` と `BEAK_EXPECTED_DIFF`〔§0-E-6〕・cat／swan の `logoRoi` と全8種の `logoRange`・`detectLogo` の keep 除外〔§0-E-8。**改訂19: swan の detectLogo 経路は失効 — keep 除外は detectLogo を用いる5種に適用・swan の logoRange は §0-G-3 の新値・旧回帰は §0-G-4 の新回帰へ置換**〕・新規検査4種〔混合所有画素数 0・外周取り残し率 ≤10.0%・grain 比局所偏差 ≤0.15・全8種の logoRange 照合〕**、さらに**改訂18分 = 検査8 再生成照合の検証 spec 合成を §0-F-1 の契約へ是正（ツール定数層の合成・一致 assert 4点〔重複一致①〜③＋logoRoi↔logoRoiWork 恒等一致④〕の新設）＋selftest ⑬〔§0-F-2〕**、加えて**改訂19分 = swan の `SWAN_OLD_LOGO_REMOVAL`〔旧筆記体の除去・§0-G-1〕と `SWAN_LOGO_TRANSPLANT`〔cat 字形転写・§0-G-2〕・CHARM_SPECS.swan の `logoThreshold: null` 化と `logoRange {778,1168,1,1}`〔§0-G-3〕・selftest ⑬ の帰属表拡張と変異⑬⑭〔計14件・§0-G-4〕**。selftest=1 全通過 → toolCommit（同期計画 §7） |
| Phase 4 全8種再生成 | 旧フェーズ0 | canonicalSource 8点から派生アセット一式を再生成（§5-4 検査1〜8・M・F・straight PNG・独立 F oracle・mutation・6ストーン位相・§7 チェックリスト・**幾何パラメータ固定 → 81配置3条件検査〔§7-11〕**・fixedVisualCaseCount の確定）。**§7 の「免除不可」項目は8種すべて合格が必須・swan の stitchMode をここで確定**。**改訂19 追加: swan は ⑤-0.5 旧ロゴ除去＋⑤ glyphMask 転写で生成（本番出力の必須 assert 5点 = finalExpect 完全一致・glyphMask 構造／SHA・removalExpect 完全一致＋range・stone keep 1,515px・**transplantRange 独立範囲判定〔logoTransplantPxCount と成分数・inspections17.logoInspection.rangeCheck へ記録〕** — §5-4 検査8 併設。provenance・定数転記照合は Phase 3 selftest 項目〔§0-G-4 ⑤⑥〕）し、swan を除く7種は rev18 assetCommit `e48dc41` の書き出し済み全 PNG〔`<key>.base.png`・`<key>.masks.png`・実在する全 `<key>.manual-*.png` — §5-2 のフラット命名〕との RGBA 完全一致を確認する（§0-G-5）** → assetCommit（同期計画 §8） |
| Phase 5 形状・意匠QA | 旧フェーズ0 承認ゲート | canonicalReference との4面比較・形状/意匠の承認（色承認と分離・**swan ロゴ立体陰影の見た目の最終判断、および改訂19の cat 字形転写後の「&.」目視承認を含む〔§0-G-6 — 還流票 visualApproval の解消〕** — AE-20260718-07）。素材検証レポート8種構成の承認 → shapeApprovalCommit（同期計画 §9） |
...
- **改訂14に伴う再実施の総括〔改訂14 当時の非規範的な履歴 — 本項の「schemaVersion 5」を現行指示として解釈してはならない。現行の schemaVersion は §0-G-5 の 7 が正〕**: 改訂13 §8 の「photo 6種は画像の再生成不要」条項は**撤回**（§0-B-3）。全8種を schemaVersion 5〔当時の値〕・straight PNG 契約で再生成し、旧派生アセット・旧 prep.json・旧 manual レイヤ（旧素材の作業座標系由来）は流用しない。81配置3条件検査・§9-2 実寸比率検証・素材検証レポートも8種構成で新規取得する〔**再実施の要求自体は改訂17 の §0-E-11「Phase 4 は全8種を再生成」（当時 schemaVersion 6・§0-E-9）を経て、現行は改訂19 の §0-G-5「全8種を v7 の生成規則で再書き出し」が正本として引き継いでいる**〕
- 計画外の変更が必要になった場合は **Codex 変更還流票（同期計画 §12）**を提出して停止し、Fable 5 の再改訂・再ロック（同 §12-1・§12-2 の restartPhase 判定）を経てから再開する

## 9. 受け入れ基準

0. **派生アセット検査**: 8種すべてが §5-4 の書き出し前検査1〜7・M・F＋コミット前検査8（再生成一致〔検証 spec は §0-F-1 の合成契約 — 改訂18 新設・**改訂19 でも不変（`logoTransplant`・`oldLogoRemoval` はツール定数層として合成対象 — §0-G-4）**〕・straight PNG 独立デコード・独立 F oracle・**mutation 6種**〔改訂17 — ⑥`shadeMin` 適用範囲誤りを追加・§5-4〕）に通過している（Phase 4 成果の再確認）。**CHARMS テーブル値と prep.json の転記一致（§5-2 の10項目の完全列挙）、LEATHER_VARIANTS 転記8項目・MATERIAL_PROFILES 転記6項目と color freeze の一致を `?debug=1` の機械照合で確認**（§0-A）
...
1. ~~本計画書を `/codex-review` で反復レビューし収束させる~~ **改訂7まで済（改訂5: 反復5回／改訂6: 反復4回／改訂7: 反復4回 — いずれも `ok: true` 収束・2026-07-12〜13）**。**改訂14は同期計画 §6-5 の revision lock 手続き（arch / diff / cross-check の3フェーズレビュー・全 ok:true・lock manifest）で確定済（2026-07-19・selectedLockCommit `2dc9420`）。改訂15は同 §12-1 の再改訂・再ロック手順で確定済（2026-07-22・selectedLockCommit `3fda5bf`・supersedesLockCommit=`2dc9420`）。改訂16は同 §12-1 の再改訂・再ロック手順（同じ3フェーズレビュー・全 ok:true・新 lock manifest・supersedesLockCommit=`3fda5bf`・restartAnchorCommit=`e71507d`〔rev15 phase-3 evidence〕）で確定済（2026-07-28・selectedLockCommit `5cc72dd`）**。
   **改訂17は同手順で確定済（2026-08-02〜03）** — restartAnchorCommit = `923e2094`（Codex 変更還流票 20260730T081002Z-osanpo-asset-fix-01 の changeTicketCommit・受領検証 28項目 ALL PASS）・supersedesLockCommit = `5cc72dd`・restartPhase = Phase 2・selectedLockCommit = `4df10c2`（§0-E 前文・§12-2 の複合分類）。
   **改訂18は同 §12-1 の再改訂・再ロック手順で確定する** — restartAnchorCommit = `cc5ce2bacce9322dee7f418c881d4ee5e05847d2`（rev17 phase-3 evidence。**還流票なしのため §12-1 手順2 の「直前に外部選択した phaseEvidenceCommit」** — 親 = `0c5dbb5` 単一・旧 selectedLockCommit `4df10c2` を祖先に持つことを検証済み）・supersedesLockCommit = `4df10c2`・restartPhase = Phase 3・sourceFreezeCommit `8b3bdb4` は carry-forward で不変・**新 documentCommit の唯一の親は restartAnchorCommit**（§0-F 前文・§12-2 の Phase 3 単独分類・§12-1 手順4）。
   **改訂19は同 §12-1 の再改訂・再ロック手順で確定する** — restartAnchorCommit = `9a9fcacf2094bddff9657d48bad4115cc405ffd1`（**還流票あり — §12-1 手順2 の「検証済み changeTicketCommit」**。ticket = `reports/charm-combo/rev18/change-tickets/20260812T060729Z-swan-logo-01.json`・ticketPayloadSha256 = `109e7ad90b8ce2e6c0f0a22b3d3deb1e7eb73794c3525bc4a04ccfed37f45f74`〔末尾 LF 方式 — §0-G 前文・§8 override 行〕・visual-comparison sha256 = `3611baead77e61556c47bf95a4b5c62923e30f44344bcc81b835070d54ea3c4a`〔いずれも §0-G 前文と同一値〕。親 = implementationCommit `cf69bf0`〔rev18 phase-4 evidence〕単一・旧 selectedLockCommit `37414ad` を祖先に持つことを検証済み〔`7ebbee9` → `b028f47` → `f598dbf` → `e48dc41` → `cf69bf0` → `9a9fcac` の一本道〕）・supersedesLockCommit = `37414ad`・restartPhase = Phase 3・sourceFreezeCommit `8b3bdb4` は carry-forward で不変・**新 documentCommit の唯一の親は restartAnchorCommit**（§0-G 前文・§12-1 手順2／手順4）
...
6. **改訂19の取り込み**: 実装エージェント（Codex）は selectedPhase2EvidenceCommit を起点とする専用 worktree（同期計画 §7-0・ブランチ `feature/charm-combo-codex-rev19`）で作業し、Phase 3 の開始条件（revision lock 照合 PASS・dirty worktree 保全・同期計画 §12-3 の再開ゲート6条件）を満たしてから着手する。**報告先は `reports/charm-combo/rev19/` へ固定し、rev14〜18 配下は読み取り専用とする（§0-G-6）**。v2 指示書と本計画書が矛盾する場合は**本計画書（改訂19）を優先**し、矛盾内容を報告する
